...
#pragma omp parallel for
    for (uint32_t curRow = 0; curRow < nc; curRow++) {
        ...
        mergeArrays(row1 + col1[curRow], symmetricRowItems[curRow], fullRow, col1[curRow + 1] - col1[curRow],
                    colSizes[curRow]);
...
#pragma omp parallel for
    for (uint32_t curCol = 0; curCol < nc; curCol++) {
        ...
        mergeArrays(row1 + col1[curCol], symmetricRowItems[curCol], fullCol, col1[curCol + 1] - col1[curCol],
                    colSizes[curCol]);
...
#pragma omp critical
        {
            for (int i = 0; i < localRes->top; i++) {
                push(res, localRes->array[i]);
            }
        }
        free(localRes);
    }
...
int main(int argc, char *argv[]) {
    ...
    runAndPresentResult(rowsCsc, colsCsc, rowsCsc, colsCsc, nc, cscOmpV1, "V1 OpenMP", "./v1-omp.txt",
                        "./v1-omp-results.txt");
