...
void cscDistributedV2(uint32_t *row1, uint32_t *col1, uint32_t *row2, uint32_t *col2, struct Stack *res, uint32_t nc, uint32_t offsetX, uint32_t offsetY) {
...
int main(int argc, char *argv[]) {
    ...
    for (uint32_t i = 0; i < blocksPerRow; i++) {
        ...
        cscDistributedV2(rowsCsc, colsCsc, rowsCsr, colsCsr, res, ncc, col * elementsPerBlock, row * elementsPerBlock);
    ...
    if (SelfTID == 0) {
        ...
        printf("| Time: %10.6lf\n", time);
        printf("-----------------------------------\n");
        saveOutput(fullRes, "v2-mpi.txt");
        saveStats("V2 MPI", time, "v2-mpi-results.txt");
        free(fullRes);
    }
